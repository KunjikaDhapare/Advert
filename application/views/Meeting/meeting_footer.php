    ...
    <script type="text/javascript">
        $(document).ready(function() {
           	<?php if($page == 'meeting'){ ?>
                ...
                $('#meeting_mgnt').addClass('active');
                document.title = "Colonia | Meeting";
           	<?php } ?>
	        ...
	        $('input[name="meeting_date"]').datepicker({
	        	minDate:"0",
                dateFormat:'d M yy',
                changeMonth: true,
                changeYear: true
            ...
            $(document).on('change','select[name="meeting_scope"]',function(data){
                var scope = $(this).val();
                // alert(scope);
                $('#meeting_MOM').removeClass();
                if(scope == 6){
                    $('#meeting_MOM').addClass('col-sm-4');
                }else{
                    $('#meeting_MOM').addClass('col-sm-4 hidden');
                }
            });

            $('#deleteMeetingRe').on('show.bs.modal', function(e) {
                var meeting_id = $(e.relatedTarget).data('id');
                $(e.currentTarget).find('input[name="meeting_id"]').val(meeting_id);
            });
