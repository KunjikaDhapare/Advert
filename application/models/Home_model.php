	...
	class Home_model extends CI_Model
	{
		public function login_check($data)
		{
			...
			if(empty($login)){
				...
				if($user['role_id'] == '1'){
					$this->db->insert('ad_user_session',['user_id'=>$login[0]['member_id'],'hash'=>$this->unique(),'islogedinn'=>1]);
		...
		public function logout_user($data)			
		{
			...
			$this->session->unset_userdata('comAdmin');
			$this->session->unset_userdata('prjHead');
			$this->session->unset_userdata('pubIncharge');
			$this->session->unset_userdata('offExe');
